...
export default function ManageTransportBookingsPage() {
  ...
  const filtered = bookings.filter((b) => {
    ...
    return (
      ...
      b.segments.some((s) => s.sector.toLowerCase().includes(q))
    );
  });
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="mb-4">
        <input
          type="text"
          placeholder="Search by guest, phone, agent, vendor, or route..."
          value={search}
                ...
                <tr key={booking.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  {Array.from({ length: maxSegments }).map((_, i) => (
                    <td key={i} className="px-4 py-3 text-gray-600">
                      {booking.segments[i] ? booking.segments[i].sector : "—"}
                    </td>
                  ))}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      <IconButton href={`/api/transport-bookings/${booking.id}/pdf?type=invoice`} title="Generate Invoice" external>
                        <FileTextIcon />
                      </IconButton>
                      <IconButton href={`/api/transport-bookings/${booking.id}/pdf?type=voucher`} title="Generate Voucher" external>
                        <TicketIcon />
                      </IconButton>
