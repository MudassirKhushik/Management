...
export default function ManageTransportBookingsPage() {
  const [bookings, setBookings] = useState<TransportBookingWithSegments[]>([]);
  ...
  async function handleQuickPayment(bookingId: string, payload: QuickPaymentPayload) {
    ...
    try {
      const res = await fetch("/api/payments", {
        ...
        body: JSON.stringify({ bookingType: "transport", bookingId, ...payload }),
      });
      if (!res.ok) throw new Error("Server rejected the payment");

      const updated = await fetch(`/api/transport-bookings/${bookingId}`);
      if (updated.ok) {
