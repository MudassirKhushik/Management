...
export default function ManageVisaBookingsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error(err);
        setError("Could not load visa bookings.");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this visa booking? This cannot be undone.")) return;
    try {
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">Manage Visa Bookings</h1>
        ...
        <table className="border w-full">
          ...
          <tbody>
            {bookings.map((booking) => {
              const grossBuying = booking.entries.reduce((sum, e) => sum + e.buyingCost, 0);
              const grossSelling = booking.entries.reduce((sum, e) => sum + e.sellingPrice, 0);
              ...
              return (
                <tr key={booking.id} className="border">
                  ...
                  <td className="border p-2">{booking.entries.length}</td>
                  ...
                  <td className="border p-2">
                    <Link className="underline mr-2" href={`/portal/visa-bookings/${booking.id}/edit`}>
                      Edit
                    </Link>
