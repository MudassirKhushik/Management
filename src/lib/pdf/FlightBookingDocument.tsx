...
//
// Voucher = client-facing, ZERO pricing shown (matches Hotel/Transport's rule).
//   PNR is kept on the voucher — it's a check-in/boarding reference, not a
//   price, same reasoning as Hotel keeping Confirmation No. on its voucher.
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.cityTitleBar}>
          <Text style={styles.cityTitle}>Flight Segments</Text>
        </View>
        <View style={styles.table}>
          ...
          {booking.segments.map((s, i) => (
            <View key={i} style={i % 2 === 1 ? styles.tableRowAlt : styles.tableRow}>
              <Text style={[styles.td, { flex: 1.4 }]}>{s.airline} {s.flightNo}</Text>
              <Text style={[styles.td, { flex: 1.8 }]}>{s.departureAirport} → {s.arrivalAirport}</Text>
              <Text style={[styles.td, { flex: 1.6 }]}>{fmtDate(s.departureDateTime)} {fmtTime(s.departureDateTime)}</Text>
              <Text style={[styles.td, { flex: 1.6 }]}>{fmtDate(s.arrivalDateTime)} {fmtTime(s.arrivalDateTime)}</Text>
              ...
              <Text style={[styles.td, { flex: 1.1 }]}>{s.pnr || "—"}</Text>
              {isInvoice && (
                <>
        ...
        {/* Voucher shows the total selling price (what the client owes) but
            never the buying cost or profit — those stay invoice-only. */}
